...
Brain = (function() {
  ...
  Brain.prototype.triggerRegexp = function(user, regexp) {
    ...
    while (match) {
      ...
      regexp = regexp.replace(new RegExp("\\s*\\[" + utils.quotemeta(match[1]) + "\\]\\s*"), "(?:" + pipes + "|(?:\\s|^)+|(?:\\s|$)*)");
      match = regexp.match(/\[(.+?)\]/);
    }
    ...
    while (regexp.indexOf("<bot") > -1) {
      ...
      if (match) {
        ...
        if (this.master._var[name]) {
          rep = utils.stripNasties(this.master._var[name], this.utf8);
        }
